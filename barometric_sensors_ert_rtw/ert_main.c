...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    barometric_sensors_M->Timing.clockTick0;
  barometric_sensors_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(barometric_sensors_M, 0);
  ...
  barometric_sensors_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(barometric_sensors_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(barometric_sensors_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(barometric_sensors_M);
