...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) pwm_test_M->Timing.clockTick0;
  pwm_test_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(pwm_test_M, 0);
  ...
  errorCode = extmodeInit(pwm_test_M->extModeInfo, &rtmGetTFinal(pwm_test_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(pwm_test_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(pwm_test_M);
